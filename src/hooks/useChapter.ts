...
export default ({
  ...
  const getChapter = async () => {
    const settings = {
      ...
      embededOverwriteOthers:
        bible.value.view.subheadings.embededOverwriteOthers,
      activeModulesSubheadings: [...bible.value.view.subheadings.activeModules],
      activeModulesCommentaries: [
        ...bible.value.view.commentaries.activeModules,
      ],
    };
    ...
    //parse text
    data.texts.forEach(
      (element: any) => (element.text = splitWords(element.text))
    );
    //stories
    ...
    data.subheadings?.forEach((subheading: any) => {
      ...
      if (!data.texts[verseNumber].subheadings)
        data.texts[verseNumber].subheadings = [];
    ...
    bookmarks.bookmarkCategories.value.forEach((category) => {
      category.bookmarks.forEach((bookmark) => {
        ...
        if (isCognateBookNumber && isCognateChapterNumber)
          for (
            let i = bookmark.startVerseNumber!;
            i <= bookmark.endVerseNumber!;
            i++
          ) {
            ...
            currentVerse.bookmarkCategories[category.name].bookmarks.push(
              bookmark
            );
          }
      });
