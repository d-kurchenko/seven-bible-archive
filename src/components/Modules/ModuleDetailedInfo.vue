  ...
  <q-dialog :model-value="show">
    <q-card>
      ...
      <q-card-section class="scroll-container" style="max-height: 50vh">
        <p><b>{{ $t('abr') }}</b>: {{ module.abr }}</p>
        <p v-if="module.lng"><b>{{ $t('lng') }}</b>: {{ module.lng }}</p>
        <p><b>{{ $t('siz') }}</b>: {{ module.siz }}</p>
        <p><b>{{ $t('des') }}</b>: {{ module.des }}</p>
        <p v-if="module.inf"><b>{{ $t('inf') }}</b>: <span v-html="module.inf"/></p>
      </q-card-section>
    </q-card>
  </q-dialog>
</template>
