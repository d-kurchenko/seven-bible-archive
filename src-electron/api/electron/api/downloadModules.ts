...
export const downloadModules = async (modules: Download[]) => {
  for (const module of modules) {
    ...
    _path = moduleType === Modules.Bible ? Path.Bible
      : moduleType === Modules.Commentaries ? Path.Commentaries
        : moduleType === Modules.Crossreferences ? Path.Crossreferences
          : moduleType === Modules.Subheadings ? Path.Subheadings
            : Path.Dictionaries
    ...
    for await (let url of module.url) {
      try {
        let downloadModuleUrl: string = ''
        url = url.replace(/{.+}/gi, (replacer, idx, all) => {
          const aliasToReplace = replacer.replace(/[{}]/gi, '')
          const serverPath = registry.hosts.find(host => host.alias === aliasToReplace)?.path!
        ...
        const fileExtension = moduleType === Modules.Bible ? '.SQLite3' : `.${moduleType}.SQLite3`
